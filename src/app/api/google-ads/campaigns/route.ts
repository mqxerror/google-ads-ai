...
import { updateCampaignStatus, updateCampaignBudget, createCampaign, CreateCampaignInput, fetchCampaigns } from '@/lib/google-ads';
...
const DEMO_CAMPAIGNS = [
  { id: '1', name: 'Brand Search', status: 'ENABLED' as const, type: 'SEARCH' as const, dailyBudget: 100, spend: 2500, clicks: 1200, impressions: 15000, conversions: 45, ctr: 8.0, cpa: 55.56, roas: 3.2, aiScore: 78, aiRecommendation: 'Consider increasing budget' },
  { id: '2', name: 'Generic Keywords', status: 'ENABLED' as const, type: 'SEARCH' as const, dailyBudget: 150, spend: 4200, clicks: 800, impressions: 25000, conversions: 12, ctr: 3.2, cpa: 350, roas: 0.8, aiScore: 35, aiRecommendation: 'High CPA - review keywords' },
  { id: '3', name: 'Shopping Feed', status: 'ENABLED' as const, type: 'SHOPPING' as const, dailyBudget: 75, spend: 1800, clicks: 650, impressions: 12000, conversions: 28, ctr: 5.4, cpa: 64.29, roas: 4.1, aiScore: 82, aiRecommendation: 'Top performer - scale up' },
  { id: '4', name: 'Display Remarketing', status: 'PAUSED' as const, type: 'DISPLAY' as const, dailyBudget: 50, spend: 950, clicks: 200, impressions: 45000, conversions: 3, ctr: 0.4, cpa: 316.67, roas: 0.5, aiScore: 28, aiRecommendation: 'Underperforming - consider removal' },
  { id: '5', name: 'Performance Max', status: 'ENABLED' as const, type: 'PERFORMANCE_MAX' as const, dailyBudget: 125, spend: 3200, clicks: 950, impressions: 32000, conversions: 52, ctr: 2.97, cpa: 61.54, roas: 2.8, aiScore: 71, aiRecommendation: 'Good performance, monitor closely' },
];
...
export async function PATCH(request: NextRequest) {
  try {
    const session = await auth();
    const body = await request.json();
    const { customerId, campaignId, status, dailyBudget } = body as {
      customerId: string;
      campaignId: string;
    ...
    if (dailyBudget !== undefined) {
      console.log(`[Campaigns API] Updating budget for campaign ${campaignId} to $${dailyBudget}`);
