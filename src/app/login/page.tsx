...
function LoginContent() {
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true);
    try {
      // Use signIn from next-auth/react which handles CSRF properly
      ...
      <div className="card p-8 max-w-md w-full mx-4">
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-lg text-red-500 text-sm">
            {error === 'OAuthAccountNotLinked'
              ...
              : error === 'Configuration'
              ? 'Server configuration error. Check OAuth credentials.'
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white hover:bg-gray-50 text-gray-800 font-medium rounded-lg border border-gray-200 transition-colors disabled:opacity-50"
          >
            {isLoading ? (
